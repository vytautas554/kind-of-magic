import { AboutMe } from './about-me.type';

export const aboutMePath = 'apie-mane';

export const AboutMeInfo: AboutMe[] = [
  {
    imageSrc: '../../assets/img/test-images/apie-mane-1.jpg',
    title: 'LABAS!\nMANO VARDAS ROMA',
    description:
      'Mano širdis priklauso veiksmui, gyvumui, tikrumui ir žinoma nuoširdumui nuotraukose. \n' +
      'Tvirtai tikiu - nėra nefotogeniškų žmonių! Tik vieniems mažiau, kitiems daugiau laiko reikia apsiprasti ir įsivažiuoti. Kažkas puikiai jaučiasi miesto centre, žmonių šurmulyje, o kažkas geriausiai jausis tik savuose namuose su vinilo plokštele.\n' +
      '\n' +
      'O kuriai žmonių grupei aš priklausau? Abiems!\n' +
      '\n' +
      'Nuo mažumės blaškydavausi nerasdama savo vietos po saule. Kartais tyli stebėtoja iš šono, kartais garsi dalyvė pačiame centre. Vieną dieną pasistrikinėdama kopiu į kalną atostogų metu, kitą praleidžiu tingiai ant sofos. Kartais su plazdančia suknele, kartais dumblinais kerzais. Šiandien švelni pastelinė ar ryški neoninė?\n' +
      'Kas aš? Apie ką aš? Na kodėl negaliu apsistoti ties vienu? Aš balta varna tarp visų normalių juodų.  \n' +
      '\n' +
      'Bet vieną dieną supratau, kad gal visai ir fainai būti ta balta varna! Mylėti visas spalvas, visas natas, visus skonius yra velniškai gerai! Gebėti prisitaikyti prie situacijos, rasti bendrą kalbą su pačiais įvairiausiais žmonėmis, nebūti tik juoda ar balta, būti visa spalvų palete - mano stiprybė! Gal vis tik aš spalvota varna??\n' +
      '\n' +
      'Keičiau tik savo požiūrį, o apsivertė visas vidinis pasaulis.\n' +
      '\n' +
      'Savo fotografija noriu perteikti ne tik gražų vaizdą, bet svarbiausia užfiksuoti tikrą jausmą. Kiekvieną fotosesiją fotografuoju taip, tarsi nuotraukas ketinu kabinti savo namuose ant sienų. Mano tikslas - nuotraukomis papasakoti jūsų istoriją. Tikrą, nesuvaidintą, nuoširdžią.\n' +
      '\n' +
      'Mano akys dairosi, o širdis kviečiasi visus - suprastus, nesuprastus, atradusius vietą po saule ar vis dar klajojančius, kuklius ir drąsius, žaismingus ar labiau santūrius, miesto žmones ir gamtos vaikus, visus visus, bet svarbiausia mylinčius save ir gyvenimą.',
    slogan: `I don't want normal, I want magic! Rezonuoja? Susirašom!`,
  },
  {
    imageSrc: '../../assets/img/test-images/apie-mane-2.jpg',
    title: 'KELI ĮDOMŪS FAKTAI APIE MANE',
    description:
      '✦ Juokais galima sakyti, kad gimiau su fotoaparatu rankose. Tamsus kambariukas su raudona šviesele, ryškalai, juostelės - vaikystės aplinka. Ir štai aš šiandien čia, nuėjusi fotografo tėčio pėdomis. \n' +
      '\n' +
      '✦ Be galo myliu šilumą. Net ir šlapia nugara ir prilipusiais prie kaktos plaukais linksėsiu galva sakydama “man gera!” \n' +
      '\n' +
      '✦ Mano svajonė turėti žadą atimančia gamta apsuptą alpakų, avių arba tiesiog visų įmanomų gyvūnų ūkį. Miesto centre. Su mcdonaldu už kampo, kad bet kuriuo dienos metu galėčiau gaut karštų bulvyčių. Sako svajot nedraudžiama 🙂\n' +
      '\n' +
      '✦ Visada buvau humanitarė, ir nei lašelio tiksliukė. Iki dabar nesuvokiu, kaip atsidūriau tame matematikos konkurse “Kengūra”. Bet hey, bent nusipirkau pieštuką už 5 litus!\n' +
      '\n' +
      '✦ Kartą teko matyti kaip gimdo karvė. Sakė geriau nesiartint, nes vaizdas nebus gražus. Bet buvo gražu! \n' +
      '\n' +
      '✦ Galutinai supratau, kad vis tik mėgstamiausias filmų žanras man yra dokumentika. O dar jeigu apie kokio nusikaltimo istoriją.. Mm.. 🙂 O “guilty pleasure” 90-00s banalios ir siaubingai lėkštos savaitgalinės romantinės komedijos.\n' +
      '\n' +
      '✦ Jeigu nebūčiau fotografė, ko gero šiuo metu dėčiau visas įmanomas pastangas, kad tapčiau pandų prižiūrėtoja/globėja/gelbėtoja. \n' +
      '\n' +
      '✦ Italijos ir Sakartvelo virtuvės yra vienas geriausių dalykų šioje žemėje.\n' +
      '\n' +
      '✦ Rūgštūs (bet tikrai rūgštūs) guminukai taip pat yra vienas geriausių dalykų šioje žemėje.\n' +
      '\n' +
      '✦ Mano prieglaudinukas šuo yra neabejotinai vienas geriausių dalykų šioje žemėje.',
    slogan: 'Kažkoks tekstas',
  },
];
